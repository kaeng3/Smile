...
def get_ai_commentary(code, name, pattern, close, rate, match_type, target_date=None, stock_dfs=None):
    ...
    prompt = (
        ...
        f"- 종목명: {name} ({code})\n"
        f"- 포착 패턴: {sub_pattern}\n"
        f"- 오늘(기준봉 출현 이후 1일차) 종가: {close_curr:,}원 ({rate:+.2f}%), 오늘(1일차) 저점: {low_today:.0f}원\n"
        ...
        f"4. 이평선 가격, 전고점/전저점 지지선, 거래량을 녹여 매수 타점과 명확한 손절 가격을 제시하세요. (※ 주의: 코멘트 작성 시 {low_today:.0f}원을 '전일 저점'이라고 잘못 부르지 마세요. '오늘 저점' 또는 '1일차 저점'이라고 명확히 표현하세요.)\n"
