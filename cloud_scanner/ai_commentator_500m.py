...
def get_ai_commentary(stock_info, target_date_str=None):
    ...
    prompt = f"""당신은 주식 분석 전문가 청혼가 에이전트 김일청입니다. 다음 종목의 차트 데이터에 대해 차읽남 소장의 실제 실전 매매 강의 바이블을 바탕으로 전문적이고 깊이 있는 투자 코멘트(약 4~5문장)를 작성해 주세요.
...
🛑 **[금지 사항]**:
- "안녕하세요", "주식 분석 전문가 김일청입니다" 등 인사말이나 자기소개는 절대 작성하지 말고, 바로 차트 분석 본론으로 시작하세요.
